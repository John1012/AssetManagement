...
description: Provides a structured 10-step workflow for migrating Android XML Views to
  Jetpack Compose. Use this skill whenever the user wants to convert an XML layout to
  Compose, migrate a Fragment or Activity UI from Views to Compose, adopt Compose
  incrementally, set up Compose dependencies and theming in an existing project, or
  asks about interoperability between Views and Compose. Also use when the user mentions
  modernizing their Android UI, replacing XML layouts, or adopting declarative UI.
