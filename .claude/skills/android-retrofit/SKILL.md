...
description: Expert guidance on setting up and using Retrofit for type-safe HTTP networking in Android. Use this skill whenever the user defines API service interfaces, sets up OkHttp clients, handles network responses or errors, configures interceptors, uploads files with multipart, works with dynamic URLs or headers, or integrates Retrofit with Hilt and kotlinx.serialization/Moshi/Gson. Also use when the user asks about REST API calls, network layers, or how to structure a repository around Retrofit.
---
